class PerlinNoiseHoudini {
    ...
    paint(ctx, geom, properties) {
        const { color, bgColor, perlinX, perlinY, perlinZ, lineWidth } =
            getProps(properties);

        const w = geom.width;
        const h = geom.height;
        const countVertical = h / Math.max(1, perlinY) + 1;
        const countHorizontal = w / Math.max(1, perlinX) + 1;
        ...
        for (let j = -perlinY; j < countVertical; j++) {
            ctx.beginPath();
...
const propsConfig = [
    { name: '--perlin-color', type: 'string', default: '#ecee81', to: 'color' },
    {
        name: '--perlin-bg-color',
        type: 'string',
        default: 'tomato',
        to: 'bgColor',
    },
    ...
    { name: '--perlin-z', type: 'number', default: 0, to: 'perlinZ' },
    {
        name: '--perlin-line-width',
        type: 'number',
        default: 1,
        to: 'lineWidth',
    },
...
function getProps(properties) {
    ...
    propsConfig.forEach((propConfig) => {
        if (properties.get(propConfig.name)) {
            switch (propConfig.type) {
                case 'number':
                    result[propConfig.to] = parseInt(
                        properties.get(propConfig.name),
                        10
                    );
                    break;
                default:
                    result[propConfig.to] = properties
                        .get(propConfig.name)
                        .toString();
            }
        } else {
            result[propConfig.to] = propConfig.default;
        }
    });
...
registerPaint('perlin-noise', PerlinNoiseHoudini);
